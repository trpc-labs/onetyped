...
export const fromType = (
  ...
  if (hasFlag(type, ts.TypeFlags.Object)) {
    ...
    if (hasObjectFlag(objectType, ts.ObjectFlags.Reference)) {
      ...
      if (hasObjectFlag(target, ts.ObjectFlags.Tuple)) {
        ...
        const tupleTypes =
          tupleType.typeArguments?.map((typeArgument, index) => {
  ...
  if (type.isUnion()) {
    const unionTypes = type.types.map((unionType) =>
      fromType(unionType, locationNode, checker),
    ) as [AnyNode, ...AnyNode[]];
    return union(unionTypes);
  }

  if (type.isIntersection()) {
    const intersectionTypes = type.types.map((type) =>
      fromType(type, locationNode, checker),
    ) as [AnyNode, ...AnyNode[]];
